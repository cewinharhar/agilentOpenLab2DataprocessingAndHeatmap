...
def main():
    ...
    datapreprocessing.add_argument( 
        "--delimiter", metavar="CSV delimiter", widget="Dropdown", choices=[",", ";", " "], help="Choose the delimiters in your CSV file. Default= ,", required=False, default=",")
    ...
    heatmap.add_argument( 
        "--delimiter_heatmap", metavar="CSV delimiter", widget="Dropdown", choices=[",", ";", "\t"], help="Choose the delimiters in your CSV file. Default= ," , required=False, default=",")
...
def run(args):      #
    ...
    if args.command == "Data_preprocessing":  ## If function depending which TAB was filled out
        ...
        DataPreprocessing(cal_csv, out_dir, screen_csv, output_name, dilution, outlier_sub, outlier_pro, delimiter) # ACHTUNG MUSS GLEICHE REIHENFOLGE HABEN WIE BEI DEF dATAPREPROCESSING
        ...
        delimiter_heatmap = args.delimiter_heatmap

        if args.add_excel:
            excel_choice = True
        else:
            excel_choice = False
        HeatmapPlotter(heatmap_file,excel_choice, heatmap_folder, heatmap_sheet, heatmap_name, heatmap_orientation, delimiter_heatmap)
        #print(heatmap_file, heatmap_sheet, excel_choice)

    ############################################################################################################################################
def DataPreprocessing(cal_csv_InFun,out_dir_InFun,screen_csv_InFun, output_name="", dilution=5.0, outlier_sub_InFun="", outlier_pro_InFun="", delimiter_InFun=","): #
    ############################################################################################################################################
    ...
    kal = pd.read_csv(pfad_kal, sep=delimiter_InFun )
    kal_original = pd.read_csv(pfad_kal, sep=delimiter_InFun)
    scr = pd.read_csv(pfad_screen, sep=delimiter_InFun)
    ...
    _ = sns.regplot(x=kalibration_tot['konz'].drop(fit_outliers_educt, errors='ignore'), y=kalibration_tot[subkonza].drop(fit_outliers_educt, errors='ignore'), label=("Substrate"), color='red')
    _ = sns.regplot(x=kalibration_tot['konz'].drop(fit_outliers_product, errors='ignore'),y=kalibration_tot[subkonzb].drop(fit_outliers_product, errors='ignore'), label=("Product"), color='blue')
...
def HeatmapPlotter(heatmap_file_InFun, excel_choice_InFun, heatmap_folder, 
    ...
    def heatmap(data, row_labels, col_labels, ax=None,
        ...
        **kwargs
        ...
        if ausrichtung == "horizontal":
            ...
            cbar.ax.set_title('Transformation [mM]')
    ...
    if ausrichtung == 'horizontal' or "":
        ...
        fig, ax = plt.subplots()
        im, cbar = heatmap(data_matrix_T, sub, sca, ax=ax, cmap="BuPu", cbarlabel="Transformation [mM]", vmin=0, vmax=int(round(np.max(data_matrix_T)+0.5))) #vmax=int(round(max(data_matrix_T)+0.5)) vmin and max stands for min and max values in konzentration bar
